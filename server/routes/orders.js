...
router.put('/:id/cancel', protect, async (req, res) => {
  try {
    ...
    // Send cancellation success email to user
    try {
      // Populate user for email
      await order.populate('user');
      const user = order.user;
      ...
      const htmlBody = `
        ...
        <h3>Items:</h3>
        <ul>${itemLines}</ul>
        <p>Your items have been returned to inventory. Thank you for using our service!</p>
      `;
      const mailOptions = {
        from: process.env.EMAIL_USER,
        to: user.email,
...
`Dear ${user.name},\n\nYour cancellation request is successful.\n\nOrder ID: ${order._id}\nDelivery Address: ${order.deliveryAddress}\nDelivery Instructions: ${order.deliveryInstructions || 'N/A'}\nPayment Method: ${order.paymentMethod}\nTotal Amount: ₹${order.totalAmount}\n\nYour items have been returned to inventory. Thank you for using our service!`,
      ...
      await transporter.sendMail(mailOptions);
      console.log('Order cancellation success email sent to user:', user.email, 'for order:', order._id);
    } catch (userMailErr) {
      console.error('Failed to send order cancellation success email to user for order', order._id, userMailErr);
    }
